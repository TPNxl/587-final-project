...
class DEAM_CQT_Dataset_With_CircShift_Sliding_Restricted(torch.utils.data.Dataset):
    def __init__(self, annot_path: str, audio_path: str, save_files: bool, transform_path: str, transform_name: str, transform_func=librosa.feature.chroma_cqt, start_s=15, dur=30, train=True):
        ...
        for i in range(-8, 8):
            self.parent_datasets.append(DEAM_CQT_Dataset_Sliding_Efficient(annot_path, audio_path, save_files, transform_path, transform_name, transform_func, start_s, dur, train))
        for i in range(-8, 9):
            self.parent_datasets[i+8].transform_array.roll_(i, dims=(2))

        self.LEN_WINDOW = 10
        test_transf = self.parent_dataset[0].calculate_transform(0, 0, self.LEN_WINDOW, save_files=False, instant=True)
        self.transform_width = 17
    ...
    def __getitem__(self, index: int):
        ...
        roll_val = int(np.floor(index / self.int_sz)) - 8
        ...
        chroma_cq, annots = self.parent_datasets[roll_val].__getitem__(new_index)
