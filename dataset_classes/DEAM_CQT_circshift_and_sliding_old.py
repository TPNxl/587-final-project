...
class DEAM_CQT_Dataset_With_CircShift_Sliding(DEAM_CQT_Dataset_Sliding_Efficient):
    def __init__(self, annot_path: str, audio_path: str, save_files: bool, transform_path: str, transform_name: str, transform_func=librosa.feature.chroma_cqt, start_s=15, dur=30, train=True):
        super(DEAM_CQT_Dataset_With_CircShift_Sliding, self).__init__(annot_path, audio_path, save_files, transform_path, transform_name, transform_func, start_s, dur, train)

        self.LEN_WINDOW = 10
        self.num_perms = self.dur * 2 - self.LEN_WINDOW + 1
        test_transf = super().calculate_transform(0, 0, self.LEN_WINDOW, save_files=False, instant=True)
        self.transform_width = test_transf.shape[1]
        self.int_sz = super().__len__()
        

    def __len__(self):
        return super().__len__() * self.transform_width
