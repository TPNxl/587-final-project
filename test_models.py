...
from old_code.DEAM_CQT_sliding_old import *
...
class TestModels(unittest.TestCase):
    def test_no_crash_on_init(self):
        ...
        m = GInvariantRNN_Model(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers, out_size=1)
        m = GInvariantLSTM_Model(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers, out_size=1)
    ...
    def test_GInv_RNN_invariance(self):
        ...
        model = GInvariantRNN_Model(input_size=20, hidden_size=25, num_layers=1, out_size=1)
        with torch.no_grad():
            for s in range(20):
                ...
                for q in range(20):
                    ...
                    output = model.forward(data)
                    assert torch.allclose(output_basic, output, atol=1e-5)
            model = GInvariantRNN_Model(input_size=20, hidden_size=25, num_layers=3, out_size=1)
            for s in range(20):
                input_data = torch.randn((5, 10, 20))
                output_basic = model.forward(input_data)
                for q in range(20):
    ...
    def test_GInv_LSTM_invariance(self):
        ...
        model = GInvariantLSTM_Model(input_size=20, hidden_size=25, num_layers=1, out_size=1)
        with torch.no_grad():
            for s in range(20):
                input_data = torch.randn((5, 10, 20))
                output_basic = model.forward(input_data)
                for q in range(20):
                    data = torch.roll(input_data, shifts=q, dims=2)
                    output = model.forward(data)
                    #print("Invariance error = ", torch.norm(output - output_basic).item())
                    assert torch.allclose(output_basic, output, atol=1e-2)
            model = GInvariantLSTM_Model(input_size=20, hidden_size=25, num_layers=3, out_size=1)
            for s in range(20):
                input_data = torch.randn((5, 10, 20))
                output_basic = model.forward(input_data)
                for q in range(20):
